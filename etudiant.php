...
<body>
    <nav>
        ...
        <img class="imgg1"src="img/infos.jpeg">
...
<input class="submit" class="dec" type="submit" name="ajout" value="Deconnexion"></form>
    ...
    <img class="imgg2"src="img/infos.jpeg">
    ...
    <img class="imgg3"src="img/infos.jpeg">
        <h2 class="titre3">Depot des fichier1s:</h2>
        <div class="depot">
        <form method="POST" action="#">
        <table>
        <tr>
           <td width=250px > <label  for="preversion" class="name">deposer la premiere version</label></td>
            <td width=300px><input class="input--style-5" type="file" name="fichier1"></td>
        </tr>
       
            <td><label for="corversion" class="name">deposer la version corrigee</label></td>
            <td><input class="input--style-5" type="file" name="fichier2"></td>
        </tr>
        <tr>
            <td><label for="presentation" class="name">deposer la presentation</label></td>
            <td><input class="input--style-5" type="file" name="fichier3"></td>
        </tr>
           
            <td><label for="attestation" class="name">deposer l'attestaion</label></td>
            <td><input class="input--style-5" type="file" name="fichier4"></td>
        </tr>
            <tr>
            <td><label for="fiche" class="name">deposer la fiche d'evalution</label></td>
            <td><input class="input--style-5" type="file" name="fichier5"></td>
        </tr>

</table>
<input class="submit" type="submit" name="ajout" value="Confirmer le depot des fichier1s">
</form>
<?php  
     if(isset($_POST['ajout'])){
         
    //traitement de la fichier1 a ajouter
	if(isset($_FILES['fichier1']) and $_FILES['fichier1']['error']==0)
	{
		...
		if ($_FILES['fichier1']['size'] >= 1000000){
			exit("Erreur, le fichier1 est volumineux");
		}
		$infosfichier1 = pathinfo($_FILES['fichier1']['name']);
		...
		{
		exit("Erreur, Veuillez inserer un fichier1 svp (extensions autorisées: pdf)");
		}
		$nom_fichier1=$id.".".$extension_upload;
		if(!move_uploaded_file($temp_name,$dossier.$nom_fichier1)){
		exit("Problem dans le telechargement du fichier1, Ressayez");
		}
		$f_name=$nom_fichier1;
        $sql1="update etudiant set version_une='".$f_name."' where apogee=$id";
    ...
    if(isset($_FILES['fichier2']) and $_FILES['fichier2']['error']==0)
	{
		...
		$infosfichier2 = pathinfo($_FILES['fichier2']['name']);
		$extension_upload = $infosfichier1['extension'];
		...
		if (!in_array($extension_upload, $extensions_autorisees))
		{
		exit("Erreur, Veuillez inserer un fichier svp (extensions autorisées: pdf)");
		}
